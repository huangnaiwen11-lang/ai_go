...
func (handler *handler) serveQuery(writer http.ResponseWriter, request *http.Request) bool {
	if request == nil || request.URL == nil || request.Method != http.MethodGet || request.URL.ForceQuery || request.URL.RawQuery != "" || request.URL.EscapedPath() != request.URL.Path {
		return false
	}
	if request.URL.Path != productsPath && !orderStatusPath.MatchString(request.URL.Path) {
		return false
	}
